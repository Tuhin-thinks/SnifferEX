...
async def consumer():
    ...
    asyncio.create_task(
        append_to_file_queue("follower_users_data.txt", file_write_queue)
    )
        ...
        async def listen():
            ...
            async for msg in ws:
                ...
                if result.get("messageType") == "sniffingResult" and (
                    ...
                    await command_queue.put(scroll_down_command)
                    await command_queue.put(scrape_follower_users_command)

                    print("[Consumer] Enqueued scroll down and scrape likers commands.")

        async def send_commands():
            command_queue.put_nowait(scrape_follower_users_command)
